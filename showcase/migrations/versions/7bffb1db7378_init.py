...
Revision ID: 7bffb1db7378
Revises: 
Create Date: 2023-10-29 20:53:42.525312
...
def upgrade() -> None:
    ...
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('acces_token', sa.String(length=320), nullable=False),
    sa.Column('user_id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_token_acces_token'), 'token', ['acces_token'], unique=True)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_token_acces_token'), table_name='token')
    op.drop_table('token')
    op.drop_table('goods')

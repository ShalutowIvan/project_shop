...
юзер залогинился
получил рефреш и аксес в куки. Рефреш записался в базу
если юзер поработал и аксес не истек, то остается рефреш
на след день ему логиниться не надо, так как с помощью рефреш токена обновится аксес и сам рефреш. Будет сделан запрос в бд, с помощью рефреша из БД сгенерится новый рефреш и аксес и они запишутся в куки, и новый рефреш запишется в базу
и получается он продолжит работу без авторизации без ввода логина и пароля

если истек аксес, то с помощью рефреша, сгенерится новый аксес и рефреш, логиниться опять же не надо

если польщзователь не будет долго заходить и истекут и аксес и рефреш, то заново логиниться

если нажать кнопку выйти, то удаляется рефреш и аксес, и надо заново логиниться.


сначала проверяем куки, если в куки пусто, идем в базу, если в базе пусто пишем логи и пароль. Если в базе есть токен, то делаем токен. получается всем юзерам будет доступен сервис , херня


сделать потом проверку входа с разных компов, это для случая если украли пароль.


непонтяно что с будет если разлогиниться..........

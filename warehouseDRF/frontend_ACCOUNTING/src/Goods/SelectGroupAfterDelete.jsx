...
export default function GroupDelete() {
    ...
    useEffect(() => {
            fetch(`http://127.0.0.1:9999/api/group_without_delete/${id}`)
                .then(res => res.json())
    ...
    const handleSubmit = async (event) => {
        ...
        try {            
            const response = await axios.post(
                `http://127.0.0.1:9999/api/select_group_to_transfer/${id}`,
                formData,
    ...
    return (
        ...
        <h1>Выберите группу в которую перенесете товар</h1>
